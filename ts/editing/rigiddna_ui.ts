 ...
 */
function resolveRigidDnaClusterIds(newElementIDs: Map<BasicElement, number>): number[] {
    let maxId = -1;
    newElementIDs.forEach((_id, e) => {
        if (typeof e.clusterId === 'number' && e.clusterId >= 0) maxId = Math.max(maxId, e.clusterId);
    });
 ...
 */
function exportSceneForRigidDna(): { topText: string; datText: string; newElementIDs: Map<BasicElement, number> } {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (clusterId !== undefined && lines[i].length > 0) lines[i] = `${lines[i]} ${clusterId}`;
    }
 ...
 */
function applyRigidDnaConf(confText: string, newElementIDs: Map<BasicElement, number>) {
    ...
    const touchedSystems = new Set<System>();
    for (let i = 0; i < byId.size; i++) {
        if (!dataLines[i]) break;
        const el = byId.get(i);
        if (!el) continue;
...
function rigidDnaClusterSummary(): string {
    let n = 0;
    const ids = new Set<number>();
    elements.forEach(e => {
        if (typeof e.clusterId === 'number' && e.clusterId >= 0) { n++; ids.add(e.clusterId); }
    });
...
function runRigidDnaOxViewCluster() {
    ...
    view.longCalculation(
        ...
        'Calculating clusters, please be patient...',
        () => { view.coloringMode.set('Cluster'); notify(rigidDnaClusterSummary()); },
    );
}
