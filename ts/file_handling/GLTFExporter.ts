...
function exportGLTF(
	...
	const materialMap: Map<number, THREE.Material> = new Map();

	const getMaterial = (hex: number, roughness: number, metalness: number) => {
	...
	const handleElement = (e: BasicElement) => {
	...
	const getExtraExportObjects = () => {
		...
		return scene.children.filter(object => {
			const exportObject = object as any;
			return (
				!excluded.has(object) &&
	...
	if (flattenHierarchy) {
		const exportedObjects: THREE.Object3D[] = [];
