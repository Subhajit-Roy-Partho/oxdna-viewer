...
async function readObservableOutputHeader(file: File): Promise<OxZstdHeader | null> {
...
async function decompressZstd(data: Uint8Array): Promise<Uint8Array> {
    const zstdLib = (window as any).fzstd;

    if (!zstdLib || typeof zstdLib.decompress !== "function") {
        throw new Error(
            "zstd decompressor is not loaded. Please include the browser zstd library before uploading .zst files."
        );
    }
...
async function decompressObservableOutput(file: File, system) {
    ...
    console.log(
        `Reading zstd-compressed oxDNA output: ${file.name} ` +
    ...
    const fakeFile = new File(
        [text],
        file.name.replace(/\.zst$/i, "").replace(/\.bin$/i, "") + ".dat",
        { type: "text/plain" }
    );
