...
class View {
    ...
    update3pMarker(e, diameter = 5, length = 1, spacing = .25) {
        ...
        if (document.getElementById("marker3pToggle").checked) {
            ...
            const q = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, -1, 0), e.getA3());
    ...
    setCenteringElementsFromSelection() {
        this.setCenteringElements([...selectedBases]);
        clearSelection();
    }
    setCenteringElements(centeringElements) {
            ...
            if (num_gds > 0) {
                ...
                for (let i = 0; i < num_gds; i++) {
                    ...
                    if (this.fluxWindowOpen)
                        view.addGraphData(oldGDIndx + i); //add to flux window sidebar if its displayed
                }
