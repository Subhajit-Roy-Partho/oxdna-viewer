...
function createHollidayJunctionWrapper() {
    let armLength = parseInt(document.getElementById('hjArmLength').value);
    if (isNaN(armLength) || armLength < 2) {
        notify("Please enter a valid arm length (>= 2)", "alert");
        return;
    }
    let seqsStr = document.getElementById('hjSequences').value.trim();
    let sequences = null;
    if (seqsStr) {
...
function createDXTileWrapper() {
    let length = parseInt(document.getElementById('dxLength').value);
    let spacing = parseInt(document.getElementById('dxSpacing').value);
...
function createThreeWayJunctionWrapper() {
    let armLength = parseInt(document.getElementById('twjArmLength').value);
    if (isNaN(armLength) || armLength < 2) {
        notify("Please enter a valid arm length (>= 2)", "alert");
        return;
    }
...
function createSingleCrossoverTileWrapper() {
    let length = parseInt(document.getElementById('scLength').value);
    if (isNaN(length) || length < 4) {
        notify("Please enter a valid tile length (>= 4)", "alert");
        return;
    }
...
function createStapleConnectorWrapper() {
    let seq = document.getElementById('scSequence').value.trim().toUpperCase();
...
function createTensegrityTriangleWrapper() {
    let edgeLength = parseInt(document.getElementById('ttEdgeLength').value);
...
function createDXLatticeWrapper() {
    let rows = parseInt(document.getElementById('dxlRows').value);
    let cols = parseInt(document.getElementById('dxlCols').value);
    let tileLength = parseInt(document.getElementById('dxlTileLength').value);
