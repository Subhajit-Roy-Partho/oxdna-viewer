...
#endif
`;
function patchInstancedVertexShader(shader) {
...
function configureInstancedMaterial(material) {
...
function applyInstancedDepthMaterials(mesh) {
...
function gpuPicker(event) {
    ...
    const x = Math.max(0, Math.min(pickingTexture.width - 1, Math.floor(((event.clientX - rect.left) / rect.width) * pickingTexture.width)));
    const y = Math.max(0, Math.min(pickingTexture.height - 1, Math.floor(((event.clientY - rect.top) / rect.height) * pickingTexture.height)));
